...
from ..experiments.sentiment_position.config import (
    SUPPORTED_FITTING_METHODS,
    SUPPORTED_FITTING_POSITIONS,
)
from ..experiments.sentiment_position.config import (
    apply_config_overrides as apply_sentiment_position_overrides,
)
from ..fitting_methods import list_fitters
from ..models import (
...
from ..selection.hyperparameters import TUNABLE_METHODS, run_confirmation, run_tuning
...
def _add_correctness_filter_arguments(
    ...
    revision_flags = (
        ("--filter-revision", "--revision") if revision_alias else ("--filter-revision",)
    )
    parser.add_argument(
        *revision_flags,
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="sentiment-geometry")
    ...
    position_comparison.add_argument(
        "--config", default="configs/sentiment_position_comparison.yaml"
    )
    position_comparison.add_argument(
        "--model",
        action="append",
        choices=("gpt2-small", "qwen-0.6b"),
        help="model to run; repeat for both (config runs both by default)",
    )
    position_comparison.add_argument(
    ...
    position_comparison.add_argument("--batch-size", type=int, default=None)
    position_comparison.add_argument("--output-dir", default=None)
    position_comparison.add_argument("--checkpoint-dir", default=None)
    position_comparison.add_argument("--seed", type=int, default=None)
    position_comparison.add_argument(
        "--method",
        action="append",
        choices=SUPPORTED_FITTING_METHODS,
        help="fitting method; repeat for all three (config runs all by default)",
    )
    position_comparison.add_argument(
        "--fit-position",
        action="append",
    ...
    layer_group.add_argument(
        ...
        action="store_true",
        help="sweep boundaries 1 through n_layers and exclude embedding boundary 0",
    )
    layer_group.add_argument(
        "--layer",
        action="append",
        type=int,
        help="debug only: run one non-embedding boundary; repeat as needed",
    )
    position_comparison.add_argument("--logistic-c", type=float, default=None)
    position_comparison.add_argument("--logistic-max-iter", type=int, default=None)
    position_comparison.add_argument("--logistic-tol", type=float, default=None)
    position_comparison.add_argument("--das-learning-rate", type=float, default=None)
    position_comparison.add_argument("--das-weight-decay", type=float, default=None)
    position_comparison.add_argument("--das-epochs", type=int, default=None)
    position_comparison.add_argument("--das-batch-size", type=int, default=None)
    position_comparison.add_argument("--das-max-grad-norm", type=float, default=None)
    position_comparison.add_argument("--sst-repo-id", default=None)
    position_comparison.add_argument("--sst-revision", default=None)
    ...
    dynasent.add_argument("--output-dir", default="data/processed/dynasent-r1-r2-pythia-2.8b")
    ...
    elif args.command == "compare-sentiment-positions":
        config = apply_sentiment_position_overrides(
            SentimentPositionExperimentConfig.load(args.config), args
        )
