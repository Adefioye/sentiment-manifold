...
PAIRING_MODEL_SPECS: dict[str, PairingModelSpec] = {
    ...
    "qwen-0.6b": PairingModelSpec("qwen-0.6b", "Qwen/Qwen3-0.6B-Base", "qwen_0_6b", False, 32768),
    "gemma-2b": PairingModelSpec("gemma-2b", "google/gemma-2b", "gemma_2b", True, 8192),
    ...
    *,
    ...
    for (split, signature), labels in sorted(buckets.items()):
        ...
        for pos, neg in zip(positive, negative):
            pair_id = (
                f"{dataset_name}-{split}-{pairing_model.replace('.', '_')}-pair-{len(matches):06d}"
            )
            match: dict[str, Any] = {
                "pair_id": pair_id,
