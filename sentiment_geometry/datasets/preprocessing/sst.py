...
def binarize_rows(rows: Iterable[dict[str, Any]], method: str) -> list[dict[str, Any]]:
...
def make_directed_pairs(matches: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for match in matches:
        for clean_polarity, corrupted_polarity in (
            ...
            directed.append(
                {
                    "case_id": (f"{match['pair_id']}-{corrupted_polarity}-to-{clean_polarity}"),
...
def preprocess_sst(
    *,
    ...
    variants = {
        method: binarize_rows(source_rows, method) for method in requested_methods[binarization]
    }
    scoring_method = (
        TIGGES_BINARIZATION if TIGGES_BINARIZATION in variants else NEUTRAL_REMOVED_BINARIZATION
    )
    ...
    if push_to_hub:
        ...
        published_repo = publish_dataset_configs(
            ...
            card_text=card_text,
            token=hf_token,
            default_repo_name=(f"sentiment-manifold-sst-{model_metadata['filter_model_alias']}"),
            metadata=metadata,
        )
