...
def _resample_editor(attention_mask: Tensor, vector: Tensor) -> Callable[[Tensor], Tensor]:
    """Create a resampling editor with immutable batch-specific state."""
    ...
    def editor(hidden: Tensor) -> Tensor:
        ...
        return torch.where(attention_mask.bool().unsqueeze(-1), replaced, hidden)
    ...
    *,
    ...
    for batch in _token_batches(adapter, texts, sequence_length, batch_size):
        ...
        with torch.inference_mode():
            ...
            editor = _resample_editor(batch["attention_mask"], vector)
