...
from ...analysis import cosine_similarity_table, projection_accuracy, projection_threshold
...
from ...evaluation import evaluate_directional_patching, evaluate_openwebtext_ablation
...
def run_reproduction(config: ReproductionConfig) -> Path:
    ...
    for layer in tqdm(layers, desc=f"{config.model.name} layer boundaries"):
        train_activations = extract_activations(
            adapter, toy_train, layer, position="focus", batch_size=config.model.batch_size
        )
        test_activations = extract_activations(
            adapter, toy_test, layer, position="focus", batch_size=config.model.batch_size
        )
        sst_activations = extract_activations(
            adapter,
