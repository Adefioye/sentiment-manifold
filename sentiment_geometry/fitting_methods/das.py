    ...
    def _prepare(
        ...
        for start in range(0, len(pairs), self.config.batch_size):
            ...
            with torch.no_grad():
                ...
                clean_final_positions = adapter.last_positions(clean.attention_mask)
                corrupted_final_positions = adapter.last_positions(corrupted.attention_mask)
                clean_logits = clean_output.logits[rows, clean_final_positions].float()
                corrupted_logits = corrupted_output.logits[rows, corrupted_final_positions].float()
