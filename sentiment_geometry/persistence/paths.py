...
def resolve_output_dir(
    ...
    *,
    ...
    drive_subdir: str = "sentiment-geometry/results",
) -> Path:
    if output_dir is not None:
        path = Path(output_dir)
    elif os.environ.get("SENTIMENT_GEOMETRY_OUTPUT_DIR"):
        path = Path(os.environ["SENTIMENT_GEOMETRY_OUTPUT_DIR"])
...
def resolve_checkpoint_dir(
    ...
    *,
    use_google_drive: bool = False,
    drive_subdir: str = "sentiment-geometry/checkpoints",
) -> Path:
    """Resolve persistent model checkpoints independently from lightweight results."""

    if checkpoint_dir is not None:
        path = Path(checkpoint_dir)
    elif os.environ.get("SENTIMENT_GEOMETRY_CHECKPOINT_DIR"):
        path = Path(os.environ["SENTIMENT_GEOMETRY_CHECKPOINT_DIR"])
    elif use_google_drive:
        path = Path("/content/drive/MyDrive") / drive_subdir
    else:
