...
from ..analysis import projection_accuracy, projection_threshold
from ..datasets import TextExample, load_toy_movie_review, pair_toy_examples
from ..evaluation import evaluate_directional_patching
from ..experiments.reproduction.config import ReproductionConfig
from ..experiments.reproduction.run import ARTIFACT_SCHEMA_VERSION, run_reproduction
...
def split_toy_training_examples(
    ...
    for label in (0, 1):
        ...
        n_validation = min(
            len(labelled) - 1,
            max(1, round(validation_fraction * len(labelled))),
        )
...
def run_tuning(config: ReproductionConfig, method: str) -> Path:
    ...
    for layer in tqdm(layers, desc=f"tune {method} layer boundaries"):
        fit_activations = extract_activations(
            adapter, fit_examples, layer, position="focus", batch_size=config.model.batch_size
        )
        validation_activations = extract_activations(
            adapter,
            validation_examples,
            layer,
            position="focus",
...
def apply_selected_configuration(
    ...
    *,
    ...
    if not isinstance(hyperparameters, dict):
        raise TypeError("Selected hyperparameters must decode to a JSON object")
