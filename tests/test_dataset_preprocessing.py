...
from sentiment_geometry.datasets.preprocessing.dynasent import (
    discover_dynasent_files,
    load_dynasent_binary,
    preprocess_dynasent,
)
from sentiment_geometry.datasets.preprocessing.imdb import imdb_rows, preprocess_imdb
...
class _WhitespaceTokenizer:
    bos_token_id = 99
    init_kwargs: ClassVar[dict[str, str]] = {"_commit_hash": "fake-revision"}
        ...
        scored.append(
            {
                **row,
                ...
                "pythia_prompt_num_tokens": len(prompt_template.format(text=row["text"]).split())
                + 1,
...
def test_multi_config_card_maps_each_config_to_its_own_directory():
    ...
    assert ("- config_name: common_directed_pairs\n  data_dir: common_directed_pairs") in card
...
def test_imdb_pipeline_filters_with_pythia_before_building_test_pairs(tmp_path, monkeypatch):
    import sentiment_geometry.datasets.preprocessing.imdb as module
...
def test_dynasent_pipeline_keeps_rounds_separate_and_filters_pairs(tmp_path, monkeypatch):
    import sentiment_geometry.datasets.preprocessing.dynasent as module
